...
WEIGHTS_DIR = f'{pathlib.Path(__file__).parent.resolve()}/weights'
# The gender model architecture
# https://drive.google.com/open?id=1W_moLzMlGiELyPxWiYQJ9KFaXroQ_NFQ
GENDER_MODEL = f'{WEIGHTS_DIR}/deploy_gender.prototxt'
# The gender model pre-trained weights
# https://drive.google.com/open?id=1AW3WduLk1haTVAxHOkVS_BEzel1WXQHP
GENDER_PROTO = f'{WEIGHTS_DIR}/gender_net.caffemodel'
...
FACE_PROTO = f"{WEIGHTS_DIR}/deploy.prototxt.txt"
# https://raw.githubusercontent.com/opencv/opencv_3rdparty/dnn_samples_face_detector_20180205_fp16/res10_300x300_ssd_iter_140000_fp16.caffemodel
FACE_MODEL = f"{WEIGHTS_DIR}/res10_300x300_ssd_iter_140000_fp16.caffemodel"
...
def load_image(resource: str) -> np.ndarray:
    """
    Open file or URL image
    """
    if os.path.isfile(resource):
        return cv2.imread(resource)
...
def predict(img: np.ndarray) -> dict:
    ...
    for i, (start_x, start_y, end_x, end_y) in enumerate(faces):
        ...
        #print(f'\t{i+1}: {label}')
        out['faces'].append({'gender': gender, 'score': gender_confidence_score})
        y_pos = start_y - 15
        while y_pos < 15:
            y_pos += 15
...
def __predict_gender(input_path: str):
    """Predict the gender of the faces showing in the image"""
    # Read Input Image
